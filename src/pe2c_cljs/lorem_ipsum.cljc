(ns pe2c-cljs.lorem-ipsum
  (:require [clojure.string :as str]))

(def lorem-ispum-paragraphs
  "https://en.wikisource.org/wiki/Lorem_ipsum_(unsourced)"
...
(def lorem-ipsum-words
  (str/split (reduce str lorem-ispum-paragraphs) #" "))

(defn words
  [n]
  (take n (cycle lorem-ipsum-words)))
(def w words)
(def w-str (comp (partial str/join " ") words))

(defn paragraphs
  [n]
  (take n (cycle lorem-ispum-paragraphs)))
(def p paragraphs)
(def p-str (comp (partial str/join " ") paragraphs))

(defn words-el
  "Hiccup words"
  [n [elem attr]]
  (map #(do [elem (merge {:key %} attr) %]) (words n)))
(def w-el words-el)

(defn paragraphs-el
  "Hiccup paragraphs"
  [n [elem attr]]
  (map #(do [elem (merge {:key %} attr) %]) (paragraphs n)))
(def par-el)

(defn rp []
  (rand-nth lorem-ispum-paragraphs))
(def random-paragraph rp)
